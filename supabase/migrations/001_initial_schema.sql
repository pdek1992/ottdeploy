...
create table if not exists private.user_credentials (
  id uuid primary key default gen_random_uuid(),
...
create index if not exists user_sessions_access_user_id_idx
  on private.user_sessions (access_user_id);

create index if not exists user_sessions_expires_at_idx
  on private.user_sessions (expires_at);

create table if not exists private.cdn_metrics_runs (
  id uuid primary key default gen_random_uuid(),
...
-- No browser policies are created for access_users or private schema tables.
-- Vercel server routes should use a server-side Supabase key for administrative access.
